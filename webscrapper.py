        ...
        if jobs_list is not None:
            ...
            if page_num == 0:
                for job in page_jobs:
                    process_and_insert_job(job, db_handler, processed_ids)
...
def process_and_insert_job(job, db_handler, processed_ids):
    ...
    if jobs["id"] not in processed_ids:
        ...
        if jobs["id"].isdigit():
            jobs["job_description"] = get_job_decription(int(jobs["id"]))
            time.sleep(1)
        try:
